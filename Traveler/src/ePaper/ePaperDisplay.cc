...
void ePaper::showTag(const String &tag) {
    display.setTextColor(GxEPD_BLACK);
    display.setTextSize(2);          // built-in font scaling factor; bump up/down to taste

    display.firstPage();
    do {
        display.fillScreen(GxEPD_WHITE);   // clear the page buffer before drawing
        display.setCursor(10, 40);         // rough vertical center for a single line; tweak once you see it
        display.print(tag);
    } while (display.nextPage());
}